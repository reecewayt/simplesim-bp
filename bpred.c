             ...
             unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredTage:
    ...
    // Initialize l1 and l2 size to zero since we aren't using
    // for TAGE we have custom multi-level tables for geometric series
    pred->dirpred.tage =
    ...
    unsigned int xor)         /* history xor address flag */
{
  ...
  case BPredTage:
    ...
    if (!(pred_dir->config.tage.hist_lengths = calloc(NUM_TAGE_TABLES - 1, sizeof(int))))
      fatal("cannot allocate TAGE history length array");

    // compute history lengths for each table, with geometric growth
    j = 0;
    for (i = 3; i >= 0; i--)
    {
      pred_dir->config.tage.hist_lengths[j] = TAGE_HISTORY_REG_SIZE >> i; // 8, 16, 32, 64
      j++;
    }

    pred_dir->config.tage.geometric_factor = TAGE_GEOMETRIC_FACTOR; // 2x (8→16→32→64)
    ...
    if (!(pred_dir->config.tage.tags = calloc(NUM_TAGE_TABLES - 1, sizeof(unsigned char *))))
      fatal("cannot allocate TAGE tags array");
    if (!(pred_dir->config.tage.usefulness = calloc(NUM_TAGE_TABLES - 1, sizeof(unsigned char *))))
    ...
    // allocate arrays for each table
    ...
    FILE *stream)                 /* output stream */
{
  ...
  case BPredTage:
    fprintf(stream, "pred_dir: %s: TAGE predictor(taken)\n", name);
    break;
                                                     ...
                                                     * used on mispredict recovery */
{
  ...
  /* This Section implments the lookup algorithm for tage; A PPM-like, tag-based branch predictor. The Tage predictor accesses all of its tables
   ...
   Then the bimodal table 0 is used as a fallback. NOTE: This implementation leverages the existing architecture for a 2 bit bimodal counter.  See lines 775-805*/
  case BPredTage:
    if ((MD_OP_FLAGS(op) & (F_CTRL | F_UNCOND)) != (F_CTRL | F_UNCOND))
    {
