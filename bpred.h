...
enum bpred_class
{
  ...
  BPredTage,     /* user-defined predictor */
  BPred_NUM
};
...
struct bpred_dir_t
{
  ...
  union
  {
    ...
    struct
    {
      ...
      int *hist_lengths;    /* [0, 8, 16, 32, 64] */
      int geometric_factor; /* 2x (8→16→32→64) */
...
struct bpred_update_t
{
  ...
  char *ptage; /* TAGE predictor counter */
  struct
  {                          /* predicted directions */
    ...
    unsigned int tage : 1;   /* TAGE predictor */
  } dir;
};
...
void bpred_dump(struct bpred_t *pred, /* branch predictor instance */
                FILE *stream);        /* output stream */
#endif
