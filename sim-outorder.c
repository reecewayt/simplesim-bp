...
/* Indirect Jump Stats */
static counter_t indirect_jumps = 0;
static counter_t ctrl_ops = 0;
static counter_t jr_addr_hits_reg = 0;
static counter_t jr_dir_hits_reg = 0;
static counter_t ctrl_op_addr_hits_reg = 0;
static counter_t ctrl_op_dir_hits_reg = 0;
static counter_t jr_used_ras = 0;
static counter_t jr_used_btb = 0;
...
{
  fprintf(stream, "\nIndirect Jump Stats:\n");
  fprintf(stream, "  total control ops: %llu\n", ctrl_ops);
  fprintf(stream, "  indirect jumps: %llu\n", indirect_jumps);
  fprintf(stream, "  jr_addr_hits: %llu\n", jr_addr_hits_reg);
  fprintf(stream, "  jr_dir_hits: %llu\n", jr_dir_hits_reg);
  fprintf(stream, "  jr_used_ras %llu\n", jr_used_ras);
  fprintf(stream, "  jr_used_btb %llu\n", jr_used_btb);
  
  fprintf(stream, "\n");
  fprintf(stream, "branch prediction accuracy: %.2f%%\n",
    (double)((ctrl_op_addr_hits_reg + ctrl_op_dir_hits_reg)- indirect_jumps) * 100 / ctrl_ops);
  fprintf(stream, "indirect jump ras accuracy: %.2f%%\n",
    (double)(jr_used_ras) * 100 / indirect_jumps);
  fprintf(stream, "indirect_jump btb accuracy: %.2f%%\n",
    (double)(jr_used_btb) * 100 / indirect_jumps);
  fprintf(stream, "indirect jump addr accuracy: %.2f%%\n",
    (double)(jr_addr_hits_reg) * 100 / indirect_jumps);
  if((jr_dir_hits_reg + jr_addr_hits_reg) != indirect_jumps)
    fprintf(stream, "FATAL ERROR: Indirect jump direct accuracy not 100%%. Indirect jump dir accuracy: %.2f%%\n",
      (double)(jr_dir_hits_reg + jr_addr_hits_reg) * 100 / indirect_jumps);
}
...
ruu_commit(void)
{
  ...
  bool_t taken, pred_taken, correct_pred = FALSE;
  ...
  while (RUU_num > 0 && committed < ruu_commit_width)
  {
    ...
    if (pred && bpred_spec_update == spec_CT && (MD_OP_FLAGS(rs->op) & F_CTRL))
    {
      bpred_update(pred,
                   ...
                   /* taken? */ MD_IS_INDIR(rs->op) ? 1 : rs->next_PC != (rs->PC + sizeof(md_inst_t)),
                   /* pred taken? */ MD_IS_INDIR(rs->op) ? 1 : rs->pred_PC != (rs->PC + sizeof(md_inst_t)),
                   /* correct pred? */ rs->pred_PC == rs->next_PC,
                   /* opcode */ rs->op,
                   /* dir predictor update pointer */ &rs->dir_update);

      /* collect indirect jump statistics */
      taken = MD_IS_INDIR(rs->op) ? 1 : rs->next_PC != (rs->PC + sizeof(md_inst_t));
      pred_taken = MD_IS_INDIR(rs->op) ? 1 : rs->pred_PC != (rs->PC + sizeof(md_inst_t));
      correct_pred = rs->pred_PC == rs->next_PC;

      /*collect all ctrl ops stats (use to recalculate accuracy)*/
      ctrl_ops++;
      if (correct_pred)
        ctrl_op_addr_hits_reg++;
      else if (pred_taken == taken)
        ctrl_op_dir_hits_reg++;

      if (MD_IS_INDIR(rs->op))
      {
        indirect_jumps++;
        if (correct_pred)
        {
          if (!rs->dir_update.dir.ras)
          {
            jr_used_btb++;
            jr_addr_hits_reg++;
          }
      ...
      if (MD_OP_FLAGS(op) & F_CTRL)
      {
        ...
        if (pred && bpred_spec_update == spec_ID)
        {
          bpred_update(pred,
                       ...
                       /* taken? */ MD_IS_INDIR(op) ? 1 : regs.regs_NPC != (regs.regs_PC + sizeof(md_inst_t)),
                       /* pred taken? */ MD_IS_INDIR(op) ? 1 : pred_PC != (regs.regs_PC + sizeof(md_inst_t)),
      ...
      if (MD_OP_FLAGS(op) & F_CTRL)
        fetch_pred_PC =
            bpred_lookup(pred,
                         ...
                         /* target address */ /* FIXME: not computed */ 1,
                         /* opcode */ op,
                         /* call? */ MD_IS_CALL(op),
...
void sim_main(void)
{
  ...
  if (fastfwd_count > 0)
  {
    ...
    enum md_opcode op; /* decoded opcode enum */
    md_addr_t target_PC;
    (void)target_PC;      /* actual next/target PC address */
    md_addr_t addr;       /* effective address, if load/store */
    int is_write;         /* store? */
    byte_t temp_byte = 0; /* temp variable for spec mem access */
    half_t temp_half = 0; /* " ditto " */
